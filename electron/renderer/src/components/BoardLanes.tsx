...
import type { BoardColumnData, StoryCardData, TaskRowData } from './storyMapTypes'
...
function BoardColumn({
  ...
  useEffect(() => {
    const validKeys = new Set(column.tasks.map((task) => task.id))
    ...
    <section className="flex w-72 shrink-0 flex-col overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--card)]">
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-2">
        {column.tasks.map((task) => {
          ...
          const taskStories = column.cards.filter((story) => story.taskId === task.id)
            ...
            <div key={taskKey} className="rounded-md border border-[var(--border)] bg-[var(--muted)]">
              <div className="flex items-center gap-1 px-2 py-1.5">
                <button
                  ...
                  aria-label={`${isExpanded ? 'Collapse' : 'Expand'} task ${task.label}`}
                >
                  <ChevronIcon expanded={isExpanded} />
                  <span className="truncate text-[12px] font-semibold tracking-[0.2px] text-[var(--foreground)]">{task.label}</span>
                ...
                <button
                  type="button"
                  onClick={() => onAddStory(column.key, task.id)}
                  ...
                  title="Add story"
                  aria-label={`Add story under ${task.label}`}
                >
                  +
                </button>
              </div>
...
export default function BoardLanes({
  ...
  return (
    <main aria-label="Board" className="flex min-w-0 flex-1 flex-col bg-[var(--background)]">
